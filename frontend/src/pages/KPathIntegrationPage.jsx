...
function KPathIntegrationPage() {
   ...
   // src/pages/KPathIntegrationPage.jsx

// 기존 fetchDestinations 부분 수정
useEffect(() => {
    if (!selectedDayTitle) {
    ...
    const fetchDestinations = async () => {
        ...
        try {
            ...
            const markers = destinations.map(dest => ({
                ...
                name: dest.name,
                notes: dest.notes || '',
                place_type: dest.place_type || 0  // 🆕 추가!
            }));
    ...
    return (
        // 🚨 구조 변경: 타이틀 헤더를 패널 컨테이너 바깥으로 이동
        <div className="kpath-page-wrapper">
            
            {/* 💥 1. 최상단 글로벌 헤더 (타이틀) */}
            <header className="kpath-global-header-wrapper">
                <h1 className="kpath-header-title">
                    K-Path Travel Idea&nbsp;
                    <svg width="3%" height="20%" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            {/* 💥 2. 패널들을 담는 메인 컨테이너 (Flexbox 적용) */}
            <div className="kpath-container-main">
                
                {/* 1. 왼쪽 일정 관리 패널 (이제는 제목이 없습니다) */}
